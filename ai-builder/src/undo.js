...
const chunkArea = (r, pad = 0) => ({
  x1: r.x1 - pad, z1: r.z1 - pad, x2: r.x2 + pad, z2: r.z2 + pad,
});

async function forceload(rcon, area, on) {
  const verb = on ? 'add' : 'remove';
  await rcon.send(`forceload ${verb} ${area.x1} ${area.z1} ${area.x2} ${area.z2}`)
    .catch(() => {});
}

// forceload only *marks* chunks; the server loads (and, out here in never-visited
...
async function waitLoaded(rcon, corners, timeoutMs = 60000) {
...
export async function snapshot(rcon, region, slot) {
  ...
  await forceload(rcon, chunkArea(vaultRegion, 16), true);
  await forceload(rcon, chunkArea(region, 16), true);
  ...
  for (const piece of splitRegion(region)) {
    ...
    const dx = vault.x + (piece.x1 - region.x1);
    const dy = vault.y + (piece.y1 - region.y1);
    const dz = vault.z + (piece.z1 - region.z1);
    ...
    // with `execute if blocks` looks stricter but is useless here: the world is
    // live, so flowing water, falling gravel and decaying leaves change blocks
    // between the copy and the check, and it fails on a perfectly good
  ...
  await forceload(rcon, chunkArea(vaultRegion, 16), false);
...
export async function restore(rcon, snap) {
  ...
  await forceload(rcon, chunkArea(vaultRegion, 16), false);
  await forceload(rcon, chunkArea(region, 16), false);
  return true;
}
